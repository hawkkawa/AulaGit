...
public class Loja {
	...
	public static void verProduto(Scanner in) {
		...
		if (prod > 0 && prod <= estoque.size()) {
			...
			if(quantidadeRemover<0) {
				System.out.println("Quantidade incorreta! Digite novamente.");
				verProduto(in);
			}
	...
	public static void verCarrinho(Scanner in) {
		...
		for (Produto x : carrinhoCompras) {
			System.out.format("%2d%20s%22.2f%20d%20.2f%n", x.getIdProduto(), x.getNome(), x.getPreco(),
					x.getQuantidade(), x.getPreco() * x.getQuantidade());
		}
		div("");
		System.out.println(
